...
if (isset( $_POST['testoEmail'], $_POST['testoPsw'])) 
{
    $email = $_POST['testoEmail'];
	...
	$stmt = $conn->prepare("SELECT password FROM utente WHERE email = ?");
	...
	{
        echo json_encode(["success" => true, "message" => "Login effettuato","email" => $email]); // Puoi passare dati extra se vuoi
    }
	else 
	{
