...
Per screen reader si intende un software che permette di convertire il testo, o più generalmente il contenuto, visualizzato su uno schermo in un output audio. Questo risulta essere particolarmente utile per persone affette da impedimenti visivi che non
...
Si è assunta come durata dello Scrum giornaliero quella di 10 minuti e qualora non fosse stato possibile comunicare dal vivo (2 dei 3 componenti sono fuorisede), si
sarebbe utilizzata la piattaforma Microsoft Teams.

\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{Materiale Multimediale/Product_backlog.png}%
    \hspace{0.05\textwidth}
    \caption{Product backlog condiviso dal teams con Excel.}
    \label{fig:immaginii}
\end{figure}
...
soprattutto considerando le differenti esigenze di cui ciascun utente necessiterà. Di maggior complessità è la comparazione con Kimap;
un aspetto tuttavia che in Kimap risulta mancante è la possibilità di poter visualizzare servizi di trasporto pubblico, come ad esempio autobus,
mezzi su cui sarà garantita l'accessibilità\cite{legge118_art27_1971}. Esploreremo questo aspetto negli sviluppi futuri.
...
Durante lo sviluppo dell'applicazione si è dato particolare importanza alla definizione dei requisiti, in maniera che questi possano generare il minor numero possibile
di fraintendimenti tra i vari componenti del team. Partendo dai \textbf{requisiti funzionali}:

\begin{enumerate}
    \item \textit{L'applicazione presenta la possibilità di registrarsi, effettuare il login e poter accedere senza account.}
    \item \textit{L'applicazione permette di poter cercare un luogo di interesse.}
    \item \textit{L'applicazione presente un aggiornamento dinamico dei punti di interesse quando ci si sposta con la mappa.}
    \item \textit{L'applicazione deve poter mostrare il nord geografico.}
\end{enumerate}

Per quanto riguarda i \textbf{requisiti non funzionali}:


\begin{enumerate}
    \item \textit{Si deve avere l'user experience migliore se si utilizza un dispositivo mobile.}
    \item \textit{L'applicazione deve avere un'interfaccia accessibile.}
    \item \textit{Un luogo di interesse sarà identificato da uno specifico colore per descriverne il livello di accessibilità.}
    \item \textit{L'applicazione deve essere disponibile nei vari store di applicazione quali Google Play Store e Apple App Store.}
    \item \textit{L'applicazione deve affidarsi, ove possibile, a tecnologie open source.}
\end{enumerate}
...
Gli store di applicazioni rappresentano una piattaforma digitale in cui vengono resi disponibili applicativi, dove questi ultimi possono avere un determinato costo oppure possono essere
gratuiti.
...
Inoltre il server espone un'API, che rappresenta l'interfaccia di comunicazione tra client e backend e consente, a seguito del corretto completamento del processo di login, l'accesso alle funzionalità dell'applicazione, inclusa la visualizzazione della mappa fornita tramite un service esterno.
...
Il "Model" si occupa di gestire ed elaborare i dati, il "View" si occupa della modalità di visualizzazione dei dati mentre il "Controller" si occupa dell'interazione dell'utente e manda queste informazioni a vista e modello.
\newline
...
\newline
L'oggetto che si occupa di navigare tra le varie schermate è il \texttt{NavController}, esso però non viene passato tra le varie schermate, ma ogni schermata evoca un evento che viene poi gestito dalla schermata principale. Ad essere passato tra le varie schermate è il ViewModel, infatti per ogni schermata, di una generica cassella di testo abbiamo:
...
Il ViewModel ha il compito di eseguire il lavoro di rete in maniera asincrona, senza bloccare l'interfaccia.Inolte ha anche la possibilità di verificare i dati immessi prima ancora di fare una chiamata di rete.
...
L'IDE utilizzato per lo sviluppo è \textbf{Android Studio}, il quale fornisce strumenti per la scrittura del codice, consente il debugging,gestisce il testing delle applicazioni e le dipendenze tramite Grandle,
ossia uno strumento di build utilizzato per automatizzare il processo di compilazione del progetto. Noi lo utilizziamo per gestire l'implementazione di librerie esterne.
...
Il linguaggio utilizzato per la realizzazione della web app è \textbf{Kotlin}.E' un linguaggio moderno e supportato per le applicazioni android.E' progetatto per essere interoperabile con Java e supporta la programmazione sia orientata agli oggetti sia a paradigmi funzionali. Inoltre è un liguaggio staticamente tipizzato ed utilizza un vasto insieme di librerie e framework per velocizzare lo sviluppo di applicazioni.
\subsection{Framework e librerie}
Per lo sviluppo dell'applicazione è stato utilizzato \textbf{Android Jetpack} al fine di garantire un'architettura scalabile e manutenibile.L'interfaccia grafica è stata realizzata interamente con \textbf{Jetpack Compose}, il quale permette di descrivere la UI come funzione dello stato corrente,ciò lo fa essere più semplice da utilizzare. Per la gestione dei dati legati alla UI è stato utilizzato \textbf{ViewModel} , il quale garantisce che i dati, nonostante possano subire variazioni, non vengono persi.La navigazione tra le diverse schermate dell'applicativo è gestita tramite \textbf{Navigation Compose}, ossia una libreria che permette di controllare il passaggio dei parametri tra le schermate in modo coerente con il pattern di navigazione di Android.Per l'integrazione con il server backend, 
è stata utilizzata la libreria \textbf{Retrofit}, il quale permette di trasformare le API HTTP in interfaccia Kotlin, automatizzando la gestione delle richieste e delle risposte, inoltre permette la conversione automatica dei dati JSON provenienti dal database in oggetti scritti in lunguaggio Kotlin.
...
Per questa applicazione il fornitore è open-source ed è \textbf{OpenFreeMap} \footnote{\url{https://tiles.openfreemap.org/styles/bright}}, che reperisce le informazini da OpenStreetMap.
Purtroppo OpenFreeMap non fornisce tutte le informazini necessarie al nostro fine.
Proprietà come \textcolor{green}{\texttt{"wheelchair"}} o \textcolor{green}{\texttt{"toilet:wheelchair"}}, importanti per verificare l'accessibilità di un luogo, non sono fornite.
Attingere direttamente alla Main API di OpenStreetMap risulterebbe troppo lento, a causa della sua natura open, infatti esso permette operazioni CRUD, perfette per la modifica ma con un'infrastruttura poco performante che non supporta query complesse.
\newline
...
Con un'interrogazione in linguaggio Overpass QL è possibile ottenere una risposta in JSON.È possibile testare le proprie query sul portale \url{https://overpass-turbo.eu/}.
\newline
...
Per la gestione remota della repository del progetto è stato utilizzato \textbf{GitHub}, ossia una piattaforma di hosting che consente di condividere il codice online, facilita il tutto attraverso le operazioni di pull request.Inoltre legge e visualizza  un file chiamato ".gitignore" il quale permette a git di evitare il caricamento di file temporanei generati automaticamente da Android Studio.
\newline
...
\begin{figure}[h]
    \begin{minipage}[t]{0.2\textwidth}
        \includegraphics[width=\textwidth]{Materiale Multimediale/Foto/Schermate prototipo/landing.png}
        \caption{Schermata iniziale}
        \label{fig:langing screen}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.2\textwidth}
        \includegraphics[width=\textwidth]{Materiale Multimediale/Foto/Schermate prototipo/log in.png}
        \caption{Schermata per effettuare il login}
        \label{fig:log in screen}
    \end{minipage}
%\end{figure}
%\begin{figure}[h]
    \hfill
    \begin{minipage}[t]{0.2\textwidth}
        \includegraphics[width=\textwidth]{Materiale Multimediale/Foto/Schermate prototipo/sign in.png}
        \caption{Schermata di registrazione}
        \label{fig:sign in screen}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.2\textwidth}
        \includegraphics[width=\textwidth]{Materiale Multimediale/Foto/Schermate prototipo/menuLaterale.png}
        \caption{menu laterale utente}
        \label{fig:side menu}
    \end{minipage}
\end{figure}
...
\begin{figure}[h]
   \begin{minipage}[t]{0.3\textwidth}
        \includegraphics[width=\textwidth]{Materiale Multimediale/Foto/Schermate prototipo/mapView.png}
         \caption{Schermata visualizzata una volta aver effettuato l'accesso all'applicazione}
        \label{fig:mappa colorata}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.3\textwidth}
        \includegraphics[width=\textwidth]{Materiale Multimediale/Foto/Schermate prototipo/luogoAccessibile.png}
        \caption{Informazione in merito ad un luogo scelto nel caso in cui sia accessibile a tutti}
        \label{fig:luogo accessibile}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.3\textwidth}
        \includegraphics[width=\textwidth]{Materiale Multimediale/Foto/Schermate prototipo/luogoNonAccessibile.png}
        \caption{Informazione in merito ad un luogo scelto nel caso in cui non sia accessibile a tutti}
        \label{fig:luogo non accessibile}
    \end{minipage}
\end{figure}

\clearpage

\subsection{Validazione del prototipo}
Per la validazione lato server del prototipo, sono stati eseguiti test manuali per verificare il corretto funzionamento delle API esposte dal backend. Questi test hanno incluso la verifica delle operazioni di registrazione e login degli utenti.
Inoltre con lo strumento open source \textbf{Grafana K6} \footnote{\url{https://k6.io/}} sono stati eseguiti test di carico sul server per valutare le prestazioni e la scalabilità del sistema.
Grafana K6 permette di simulare un numero elevato di richieste simultanee al server con l'utilizzo di uno script in linguaggio JavaScript.
Ecco una simulazione nell'immagine \ref{fig:simuazioneK6} eseguita con un carico di 20 utenti con 1 richiesta di login al secondo per utente:
\begin{figure}[h]
    \centering
    \includegraphics[width=0.80\textwidth]{Materiale Multimediale/Foto/test_di_validazione_k6.png}
    \hspace{0.05\textwidth}
    \caption{Test di validazione con Grafana k6. Il server ha gestito con successo tutte le richieste senza errori, con un tempo medio di risposta di 61.03 ms.}
    \label{fig:simuazioneK6}
\end{figure}
...
Il progetto fino ad ora realizzato risulta essere soltanto una parziale versione di quella che si ha idea di sviluppare. Al momento della stesura di questo documento
tutti i membri del team dichiarano di essere intenzionati e volenterosi di continuare a collaborare con il fine di migliorare il progetto, motivo per il quale è necessario avere una direzione
su cosa andare a modificare o addirittura implementare nuovamente.

\section{Aggiunta di un sito web}

Potrebbe risultare agevole includere un sito web che offra la possibilità di essere notati sia da nuovi possibili utenti che da ipotetici stakeholder.
Al momento della stesura del seguente report, si pensa che con la versione web comunque si potrà interagire con la mappa e i vari luoghi di interesse,
tuttavia sarà consigliato la versione per dispositivi mobili in quanto la nostra visione di progetto, come detto in precedenza, è sempre stata finalizzata ai dispositivi mobili.
L'aggiunta di un sito web renderà la trattazione sulla stima dei costi effettuata obsoleta, in quanto andrebbero inclusi i costi come quelli relativi al dominio oppure al certificato SSL.
\section{Calcolo del percorso ottimale}
Si pensa che sia necessario implementare una funzionalità che permetta di calcolare il percorso ottimale per raggiungere un luogo di interesse, tenendo conto delle esigenze specifiche
dell'utente riguardo l'accessibilità, ad esempio per persone in sedia a rotelle bisogna evitare scalinate o terreni accidentati.
\section{Inserimento delle recensioni}
Un'altra funzionalità che si ritiene sia necessaria implementare è la possibilità di inserire recensioni per ciascun luogo di interesse, esclusivamente da parte di utenti registrati.
Questo consentirebbe di ottenere un feedback più accurato e affidabile riguardo all'accessibilità dei luoghi, poichè le recensioni verrebbero monitorate e moderate dal team di sviluppo, 
assicurandosi che l'utente registrato abbia effettivamente visitato il luogo in questione.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.70\textwidth]{Materiale Multimediale/Foto/costi_annuali_dominio.png}
    \hspace{0.05\textwidth}
    \caption{Verifica di disponibilità del dominio, i prezzi sono annuali.}
    \label{fig:simuazioneK6}
\end{figure}

\section{Aggiunta informazioni sui mezzi di trasporto}

Come citato nella sezione relativa all'analisi comparativa tra la soluzione corrente e Kimap, si era menzionata la questione
mezzi di trasporto pubblico, e di come informazioni relativi a questi potrebbero essere di particolare interesse per persone affette da impedimenti. Per quanto riguarda la città
di Bari, esiste una API che permette di includere nella applicazione proposta informazioni riguardanti il servizio di trasporto pubblico AMTAB. Questa implementazione permetterà di ottenere
dati aggiornati relativamente alla posizione dello specifica tratta come anche informazioni relative agli orari. Il formato in cui AMTAB
pubblica i suoi dati sono GTFS-RT.
